...
export default function GroupInfoPage() {
    ...
    const handleSave = async () => {
        if (!isCurrentUserOwner || !groupDocRef) return;
        ...
        try {
            ...
            participantIds.forEach(pid => {
                const contactRef = doc(firestore, `users/${pid}/contacts/${groupId}`);
    ...
    const uploadCroppedImage = async (croppedImage: string): Promise<string> => {
        ...
        const storageRef = ref(storage, `group-avatars/${groupId}/${Date.now()}.jpeg`);
        const snapshot = await uploadString(storageRef, croppedImage, 'data_url', { contentType: 'image/jpeg' });
        return getDownloadURL(snapshot.ref);
    };


    if (isGroupLoading || areUsersLoading) {
        return <div className="h-full flex items-center justify-center"><LoaderCircle className="h-8 w-8 animate-spin"/></div>
    }

    if (!group) {
        return <div className="h-full flex items-center justify-center"><p>Group not found.</p></div>
    }
